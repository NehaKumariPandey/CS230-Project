...
struct cluster_node_s {
        ...
        bitset<8> centroid; /* centroid of this cluster */
...
float euclidean_distance(bitset<8> a, bitset<8> b)
{
	bitset<8> x = a^b;
	bitset<8> y = a|b;
    if (y.count()==0){
      cout<<"Bitvector cannot be zero. apk must have some feature atleast."<<endl;
    } 
    //cout<<"x.count is: "<<x.count()<<endl;
    //cout<<"y.count is: "<<y.count()<<endl;
    float xc = x.count();
    float yc = y.count();
                              ...
                              const item_t items[])
{
        for (int i = 0; i < num_items; ++i){
                for (int j = 0; j < num_items; ++j) {
                        matrix[i][j] =
                                euclidean_distance(items[i].coord, items[j].coord);
...
// return dist between the 2 centroids
/*
